...
const ProfileInfo = ({ users }) => {
  ...
  const [coverPicture, setCoverPicture] = useState(users.cover_picture || "../images/profile/profile-cover-image2.png");
  const [joinedAgo, setJoinedAgo] = useState('');

  useEffect(() => {
    // Calculate the duration since the user joined
    const joinedDate = new Date(users.created_at);
        ...
        <div className="user-info">
          ...
          <div className="contact-info">
            <div className="contact-address">
              <p><i className='bx bxs-map' style={{color:'#fdb514'}}></i>{users.address}</p>
              <small className="data-description">Address</small>
            </div>
            <div className="contact-mail">
              <p><i className='bx bxs-envelope' style={{color:'#fdb514'}}></i>{users.email}</p>
              <small className="data-description">Email</small>
            </div>
            <div className="contact-number">
              <p><i className='bx bxs-phone' style={{color:'#fdb514'}}></i>{users.phone}</p>
